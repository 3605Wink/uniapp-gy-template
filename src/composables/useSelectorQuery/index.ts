...
export function useSelectorQuery(instance?: ComponentInternalInstance | null) {
	...
	if (!instance) console.warn("useSelectorQuery", "useSelectorQuery必须在setup函数中使用");
			...
			if (query) {
				query
					...
					.boundingClientRect(res => {
						...
						if (selectRes) resolve(selectRes);
						else reject(new Error(`未找到对应节点: ${selector}`));
					})
					.exec();
			} else {
	...
	const getSelectorNodeInfos = (selector: string): Promise<UniApp.NodeInfo[]> => {
		return new Promise((resolve, reject) => {
			if (query) {
				query
					.selectAll(selector)
					.boundingClientRect(res => {
						const selectRes: UniApp.NodeInfo[] = res as UniApp.NodeInfo[];
						if (selectRes && selectRes.length > 0) resolve(selectRes);
						else reject(new Error(`未找到对应节点: ${selector}`));
					})
					.exec();
			} else {
	...
	const getSelectorNodeInfoExec = (selectors: string[]): Promise<UniApp.NodeInfo[]> => {
		return new Promise((resolve, reject) => {
			if (query) {
